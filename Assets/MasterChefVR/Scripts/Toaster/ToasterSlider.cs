...
/// </summary>
public class ToasterSlider : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (hasReset) {
          shouldLock = true;
          moveablePart.GetComponent<Outline>().OutlineWidth = 2;
        }

        // Toaster logic
        if (moveablePart.transform.position.y - slideEndPoint.position.y < 0.05f && shouldLock && toasterBody.GetComponent<Toaster>().leftPopperOccupied)
        {
          hasReset = false;
          shouldLock = false;

          Lock();
          toasterBody.GetComponent<Toaster>().Toast();
          
          moveablePart.GetComponent<Outline>().OutlineWidth = 0;
        }
    }
